...
def check_created():
    url = 'http://api.eventfinda.co.nz/v2/events.json?created_since=%s'%(createddate)
    username = 'admin';
    password = 'admin';
...
def download_created(offset):
    url = 'http://api.eventfinda.co.nz/v2/events.json?created_since=%s&rows=10&offset=%d'%(createddate,offset)
    username = 'admin';
    password = 'admin';
    request = urllib2.Request(url)
    base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
    ...
    with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/events/events_data/events_nz_created_%s.json"%(createddate), "a") as fd:
...
def read_created():
    try:
        with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/events/events_data/events_nz_created_%s.json"%(createddate), "r") as rd:
            while(1):
                line = rd.readline()
                if(len(line)>0):
                    data_handle(line)
...
def data_handle(line):
    ...
    for events in events:
        ...
        cur.execute("""
                        select id from events_nz_eventfinda
                        where id = {}
                    """.format(eid))
        result = cur.fetchone()
        if(result != None):
            ...
            cur.execute("""
                        Insert into events_nz_eventfinda
                        (
                            id,
                            name,
                            presented_by,
...
def check_modified():
    url = 'http://api.eventfinda.co.nz/v2/events.json?modified_since=%s'%(modifieddate)
    username = 'admin';
    password = 'admin';
    request = urllib2.Request(url)
    base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
...
def download_modified(offset):
    url = 'http://api.eventfinda.co.nz/v2/events.json?modified_since=%s&rows=10&offset=%d'%(modifieddate,offset)
    username = 'admin';
    password = 'admin';
    request = urllib2.Request(url)
    base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
    ...
    with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/events/events_data/events_nz_modified_%s.json"%(modifieddate), "a") as fd:
        fd.write(json.dumps(data) + "\n")
    fd.close()
...
def read_modified():
    try:
        with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/events/events_data/events_nz_modified_%s.json"%(modifieddate), "r") as rd:
            while(1):
                line = rd.readline()
                if(len(line)>0):
                    data_handle(line)
...
def add_regions_local_timestamp():
    print("Truncate events_nz_eventfinda_with_regions")
    cur.execute("""Truncate events_nz_eventfinda_with_regions""")
    print("Delete Completed")
    print("Number of Rows Deleted : ", cur.rowcount)
    conn.commit()

    print("Load into events_nz_eventfinda_with_regions")
    cur.execute( """
                    insert into events_nz_eventfinda_with_regions
                    (
                        id,
                        name,
                        presented_by,
                    ...
                    from events_nz_eventfinda a, nz_tourism_regions b
    ...
    cur.execute("""
                    Update events_nz_eventfinda_with_regions
                    set create_time_local = create_time_utc + INTERVAL '12 hours'""")
    ...
    cur.execute("""
                    Update events_nz_eventfinda_with_regions
                    set last_updated_local = last_updated_utc + INTERVAL '12 hours'""")
    print("Update Completed")
    print("Number of Rows Updated : ", cur.rowcount)
    conn.commit()
