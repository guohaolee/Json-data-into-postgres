...
def check_created():
    url = 'http://api.eventfinda.com.au/v2/locations.json?created_since=%s'%(createddate)
    username = 'admin';
    password = 'admin';
...
def download_created(offset):
    url = 'http://api.eventfinda.com.au/v2/locations.json?created_since=%s&rows=10&offset=%d'%(createddate,offset)
    username = 'admin';
    password = 'admin';
    request = urllib2.Request(url)
    base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
    ...
    with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/location/location_data/locations_au_created_%s.json"%(createddate), "a") as fd:
...
def read_created():
    try:
        with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/location/location_data/locations_au_created_%s.json"%(createddate), "r") as fd:
            while(1):
                line = fd.readline()
                if(len(line)>0):
                    data_handle(line)
...
def data_handle(line):
    ...
    for data in locations:
        ...
        cur.execute("""
                        select id from location_au_eventfinda
                        where id = {}
                    """.format(lid))
        result = cur.fetchone()
        ...
        if (result != None): 
            ...
            cur.execute("""
                            Insert into location_au_eventfinda
                            (   
                                id,
                                location_name,
                                location_address,
...
def check_modified():
    url = 'http://api.eventfinda.com.au/v2/locations.json?modified_since=%s'%(modifieddate)
    username = 'admin';
    password = 'admin';
    request = urllib2.Request(url)
    base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
...
def download_modified(offset):
    url = 'http://api.eventfinda.com.au/v2/locations.json?modified_since=%s&rows=10&offset=%d'%(modifieddate,offset)
    username = 'admin';
    password = 'admin';
    request = urllib2.Request(url)
    base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
    ...
    with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/location/location_data/locations_au_modified_%s.json"%(modifieddate), "a") as fd:
        fd.write(json.dumps(data) + "\n")
    fd.close()
...
def read_modified():
    try:
        with open("/home/ec2-user/eventfinda/eventfinda_virtualenv/projects/location/location_data/locations_au_modified_%s.json"%(modifieddate), "r") as fd:
            while(1):
                line = fd.readline()
                if(len(line)>0):
                    data_handle(line)
